...
//calculate how many bytes
uint64_t Command_parse::memory_parse(std::string str, const cxxopts::ParseResult &result){
  auto nmemory = result[str].as<std::string>();
  uint64_t nmemory_byte;
  if( nmemory[nmemory.length()-1]=='M' ||  nmemory[nmemory.length()-1] == 'm'){
    nmemory_byte =(uint64_t)std::stoi(nmemory.substr(0,nmemory.length()-1)) * 1024;
...
void Command_parse::Write_StrmArg(std::string str, const cxxopts::ParseResult &result, std::ofstream &infile){
...
void Command_parse::Write_UintArg(std::string str, const cxxopts::ParseResult &result, std::ofstream &infile){
...
void Command_parse::Write_VetArg(std::string str, const cxxopts::ParseResult &result, std::ofstream &infile){
...
void Command_parse::Write_Result(std::string str, const cxxopts::ParseResult &result){
...
cxxopts::ParseResult Command_parse::parse(int argc, const char* argv[])
{
...
void Command_parse::Write_StrArg(std::string str, const cxxopts::ParseResult &result, std::ofstream &infile){
