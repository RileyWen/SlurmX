...
class KeepAliveGreeterClient {
 public:
  KeepAliveGreeterClient(
    ...
    SLURMX_INFO("Channel initial conn state: {}",
  ...
  std::string SayHello(const std::string& user) {
    ...
    if (status.ok()) {
      ...
      SLURMX_INFO("{}:{}", status.error_code(), status.error_message());
      return "RPC failed";
    }
  }
 ...
 private:
  void StateMonitorThreadFunc() {
    SLURMX_INFO("StateMonitorThread started...");
    ...
    SLURMX_INFO("StateMonitorThread is exiting...");
  }
...
TEST(KeepAlive, ServerNormallyExit) {
  ...
  auto state_change_cb = [](grpc_connectivity_state old_state,
    ...
    SLURMX_INFO("Underlying conn state changed: {} -> {}",
  ...
  SLURMX_INFO("SayHello(Riley) -> {}", reply);
